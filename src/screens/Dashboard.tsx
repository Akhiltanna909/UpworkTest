...
export const Dashboard = () => {
  ...
  const [questionList, setQuestionList] = useState<Excersice[]>([]);
  const [currentQuestion, setCurrentQuestion] = useState<number>(0);
  const [engSplitSentence, setEngSplitSentence] = useState<string[]>([]);
  const [gerSplitSentence, setGerSplitSentence] = useState<string[]>([]);
  const [selectedQuesIndex, setSelectedQuesIndex] = useState<number>(0);
  const [engLangIndex, setEngLangIndex] = useState<number>(0);
  const [gerLangIndex, setGerLangIndex] = useState<number>(0);
            ...
            {!answerChecked ? (
              <View style={styles.buttonView}>
                <Button
                  ...
                  disabled={!resetQuestion ? !selectedChoice : false}
                  onPress={() => {
